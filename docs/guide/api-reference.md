...
| `--redownload-fake-hires` | | `False` | Acts on a `--verify-hires` finding instead of only logging it: the flagged file is set aside, the track is downloaded again at `LOSSLESS`, and the flagged file is deleted only once the replacement is on disk (if every extension fails, the original is restored). Implies `--verify-hires`, and only applies when a Hi-Res quality was requested — see [Hi-Res Verification](configuration.md#hi-res-verification). |
