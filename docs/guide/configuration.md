...
- **A false positive costs bit depth.** Suspect findings are reported and kept. Certain or likely findings are eligible for replacement, but can still be wrong: a real 24-bit Hi-Res file can be replaced with a 16-bit `LOSSLESS` one if the spectral evidence is misgraded, an artifact-only finding is called certain, or a likely finding is based on a misread noise floor. A finding that names the bit depth instead ("declares 24-bit but only 16 bits carry data") is not a judgement call, and replacing that file loses nothing at all. Nothing audible is lost in that case, but it is a real trade — leave the option off and read the warnings if you would rather decide track by track.
